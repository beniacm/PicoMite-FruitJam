MODE 5
' Palette FIRST, then CLS
MAP 0 = RGB(0,0,16)
MAP 1 = RGB(255,255,255)
MAP 2 = RGB(255,0,0)
MAP 3 = RGB(255,128,0)
MAP 4 = RGB(255,255,0)
MAP 5 = RGB(0,255,0)
...
MAP 7 = RGB(0,0,255)
MAP 8 = RGB(255,0,255)
MAP 9 = RGB(128,128,128)
MAP SET
CLS RGB(0,0,16)
FRAMEBUFFER CREATE
' ASM blitter: R0=fb, R1=sprite(w,h,data), R2=x, R3=y
DIM INTEGER blt%(50)
ASM blt%()
  PUSH {R4, R5, R6, R7, LR}
  SUB SP, SP, #20
...
rd:
  ...
  POP {R4, R5, R6, R7, PC}
END ASM
' CLS helper: fill fb with byte 0
DIM INTEGER clr%(10)
ASM clr%()
  PUSH {R4, LR}
  MOV R1, #0
...
cl:
  ...
  POP {R4, PC}
END ASM
' 8x8 sprite: circle using palette indices
DIM INTEGER sdata%(8)
...
For i=0 To 7 : For j=0 To 7
  ...
  If d < 1.5 Then c% = 1
  ElseIf d < 2.5 Then c% = 4
  ElseIf d < 3.5 Then c% = 2
  Else c% = 0
  End If
